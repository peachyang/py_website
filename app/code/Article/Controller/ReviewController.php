...
use Seahinet\Article\Model\Product\Review;
...
class ReviewController extends ActionController
{
    ...
    {
        $root = $this->getLayout('article_review');
    ...
    public function saveAction()
    {
        if ($this->getRequest()->isPost()) {
            ...
            if (!$this->getContainer()->get('config')['article/review/allow_guests'] && !$segment->get('hasLoggedIn')) {
                return $this->redirectReferer();
            }
            $data = $this->getRequest()->getPost();
            $result = $this->validateForm($data, ['article_id', 'content']);
            $data['customer_id'] = $segment->get('hasLoggedIn') ? $segment->get('customer')->getId() : null;
            $data['status'] = (int) $this->getContainer()->get('config')['article/review/status'];
        ...
        return $this->response($result ?? ['error' => 0, 'message' => []], $this->getRequest()->getHeader('HTTP_REFERER'), 'article');
    }
