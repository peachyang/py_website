    ...
    $(function () {
        ...
        if (lang) {
            ...
            if (/^(?:de|es\-CL|fi|nl|pt)/.test(lang)) {
                ...
                os.async = 'async';
                os.src = url + '/js/validate/localization/methods_' + lang.replace(/^(de|es_CL|fi|nl|pt)\_.+$/, '$1') + '.min.js';
                document.head.appendChild(os);
            }
            if (/^(?:bn\-BD|es\-AR|es\-PE|hy\-AM|pt\-BR|pt\-BT|sr\-lat|zh\-TW)$/.test(lang)) {
                var os = document.createElement('script');
                os.async = 'async';
                os.src = url + '/js/validate/localization/messages_' + lang + '.min.js';
                document.head.appendChild(os);
                ...
                var os = document.createElement('script');
                os.async = 'async';
                os.src = url + '/js/validate/localization/messages_' + lang.replace(/^([a-z]{2})\-.+$/, '$1') + '.min.js';
                document.head.appendChild(os);
            }
        }
        $('form').each(function () {
