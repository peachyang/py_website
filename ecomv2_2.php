...
CREATE TABLE IF NOT EXISTS `promotion_condition`(
    ...
    `identifier` INTEGER NOT NULL COMMENT 'Identifier',
    `operator` VARCHAR(20) NOT NULL COMMENT 'Operator',
    `value` VARCHAR(255) NOT NULL COMMENT 'Value',
    PRIMARY KEY (`id`),
    INDEX IDX_PROMOTION_CONDITION_PROMOTION_ID (`promotion_id`),
    CONSTRAINT FK_PROMOTION_CONDITION_PROMOTION_ID FOREIGN KEY (`promotion_id`) REFERENCES `promotion` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
);
...
CREATE TABLE IF NOT EXISTS `promotion_coupon`(
    ...
    INDEX IDX_PROMOTION_COUPON_CUSTOMER_ID (`customer_id`),
    INDEX IDX_PROMOTION_COUPON_STORE_ID (`store_id`),
    ...
    CONSTRAINT FK_PROMOTION_COUPON_STORE_ID FOREIGN KEY (`store_id`) REFERENCES `core_store` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT FK_PROMOTION_COUPON_CUSTOMER_ID_CUSTOMER_ENTITY_ID FOREIGN KEY (`customer_id`) REFERENCES `customer_entity` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
);
...
CREATE TABLE IF NOT EXISTS `promotion_customer_group`(
    `promotion_id` INTEGER NOT NULL COMMENT 'Promotion ID',
    `group_id` INTEGER NOT NULL COMMENT 'Customer Group ID',
    PRIMARY KEY (`promotion_id`,`group_id`),
    CONSTRAINT FK_PROMOTION_CUSTOMER_GROUP_PROMOTION_ID_PROMOTION_ID FOREIGN KEY (`promotion_id`) REFERENCES `promotion` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT FK_PROMOTION_CUSTOMER_GROUP_GROUP_ID_CUSTOMER_GROUP_ID FOREIGN KEY (`group_id`) REFERENCES `customer_group` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
);
