...
namespace Seahinet\Resources\Model;
...
class Resources extends AbstractModel
{
    ...
    protected function _construct()
    {
     ...
     public function getTargetPath($fileType)
     {
         ...
         if(isset($this->options['file_types'][intval($fileType)])&&$this->options['file_types'][intval($fileType)]!=''){
             if (!is_dir($this->options['upload_dir'].$this->options['file_types'][intval($fileType)])) {
                 mkdir($this->options['upload_dir'].$this->options['file_types'][intval($fileType)],$this->options['mkdir_mode'],true);
             }
             if(intval($fileType==1)){
                 foreach ($this->options['image_versions'] as $k=>$v){
                     if(!is_dir($this->options['upload_dir'].$this->options['file_types'][intval($fileType)].'/'.$k)){
                         mkdir($this->options['upload_dir'].$this->options['file_types'][intval($fileType)].'/'.$k,$this->options['mkdir_mode'],true);
                     }
                 }
             }
         }else{
         ...
         return $this->options['upload_dir'].$this->options['file_types'][intval($fileType)].'/';
     }
