...
use DOMElement;
use DOMXPath;
use Seahinet\Lib\Http\Uri;
use Seahinet\Api\Model\Soap\Wsdl\ComplexTypeStrategy\ComplexTypeStrategyInterface as ComplexTypeStrategy;
...
class Wsdl
{
    ...
    $name, $uri, ComplexTypeStrategy $strategy = null, array $classMap = []
    )
    {
    ...
    $binding, $name, $input = false, $output = false, $fault = false, $soapVersion = SOAP_1_1
    )
    {
        $operation = $this->dom->createElementNS(self::WSDL_NS_URI, 'operation');
    ...
    $binding, $style = 'document', $transport = 'http://schemas.xmlsoap.org/soap/http', $soapVersion = SOAP_1_1
    )
    {
    ...
    public function addSoapOperation($operation, $soapAction, $soapVersion = SOAP_1_1)
    {
        if ($soapAction instanceof Uri) {
            $soapAction = $soapAction->__toString();
        }
