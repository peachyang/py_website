...
class Product extends Entity
{
    ...
    public function getCategories()
    {
        if ($this->getId()) {
            ...
            $result = $tableGateway->select(['article_id' => $this->getId()])->toArray();
            $valueSet = [];
    ...
    public function getLinkedProducts($type)
    {
        if ($this->getId()) {
            ...
            $result = $tableGateway->select(['article_id' => $this->getId(), 'type' => substr($type, 0, 1)])->toArray();
            $valueSet = [];
            array_walk($result, function($item) use (&$valueSet) {
                $valueSet[] = $item['linked_article_id'];
            });
            if (count($valueSet)) {
    ...
    {
        $constraint = ['article_id' => $this->getId()];
    ...
    protected function afterSave()
    {
        if (!empty($this->storage['category'])) {
            ...
            $maxCount = (int) $this->getContainer()->get('config')['article/product/count_in_category'];
            foreach ((array) $this->storage['category'] as $category) {
                if ($maxCount--) {
                    $tableGateway->insert(['article_id' => $this->getId(), 'category_id' => $category]);
                }
                if ($maxCount === 0) {
                    break;
                }
        ...
        if (isset($this->storage['article_link'])) {
            $tableGateway = $this->getTableGateway('article_link');
            $tableGateway->delete(['article_id' => $this->getId()]);
            if (is_array($this->storage['article_link'])) {
                foreach ($this->storage['article_link'] as $type => $link) {
                    foreach ($link as $order => $id) {
                        $tableGateway->insert([
                            'article_id' => $this->getId(),
                            'linked_article_id' => $id,
